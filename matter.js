...
function runMatter() {
  ...
  for (var i = 0; i < 30; i += 1) {  // Reduced number of bodies for performance
    ...
    var circle = Bodies.circle(x, y, Common.random(2, 8), {
      mass: 0.1,
      friction: 0,
      frictionAir: 0.005,  // Reduced frictionAir for smoother motion
      render: {
  ...
  Events.on(engine, "afterUpdate", function () {
    ...
    Body.translate(attractiveBody, {
      x: (mouse.position.x - attractiveBody.position.x) * 0.08,  // Slowed down for smoother tracking
...
function setWindowSize() {
  ...
  if (dimensions.width !== newWidth || dimensions.height !== newHeight) {
    ...
    m.render.canvas.width = newWidth;
    m.render.canvas.height = newHeight;
  }
}

let m = runMatter();
setWindowSize();
$(window).resize(debounce(setWindowSize, 150));  // Reduced debounce time for better responsiveness
